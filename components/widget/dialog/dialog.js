...
(function(root,factory){
	...
	__dialog.prototype={
		...
		__creat:function(){
			...
			modal.id=OP.id;
			modal.className=OP.className?"ui-dialog "+OP.className:"ui-dialog";
			modalWrap.className="ui-dialog-wrap";
			str=OP.title?"<div class='ui-dialog-head'><h5>"+OP.title+"</h5></div>":"";
			if(OP.content){
				str+="<div class='ui-dialog-content'>"+OP.content+"</div>"
			}
			if(OP.footer){
				str+="<div class='ui-dialog-footer'>"+(function(){
						var str="";
			...
			if(OP.closeSwitch){
				str+="<div class='ui-dialog-close'><i class='font-icon close'></i></div>"
			}
		...
		__open: function (callBack) {
			var _this=this,
				el=this.el,
				OP=this.options,
				dialogWrap=el.querySelector(".ui-dialog-wrap"),
				elHeight,
			...
			el.classList.add("in");
			if(OP.closeSwitch){
				el.querySelector(".ui-dialog-close").addEventListener("click",_this.close.bind(_this),false);
				OP.closeSwitch=false;//避免多次绑定
			}
			if(OP.blindEvent){
	...
	var DIALOG=(function(){
		var obj={},
			bgIndex=[];
		function showBg(){
			var bg=getEle(".ui-modal-bg");
			var bo=getEle("body");
			if(!bg){
				var div=DOC.createElement("div");
				div.className="ui-modal-bg"	;
				DOC.body.appendChild(div);
				bg=getEle(".ui-modal-bg")
			}
		...
		return {
			dialog:function(options,isMsg){
				...
				if(!obj[Id]){
					...
					if(isMsg=="msg"){
						newDialog.el.querySelector(".ui-dialog-content").innerHTML=options.content;
					}else{
				...
				newDialog.open=function(callBack){
					...
					currZIndex+=100;
					for(var dialogs=DOC.querySelectorAll(".ui-dialog"),l=dialogs.length;l>0;l--){
			...
			tipDialog:function(msg){
				var _msgDialog=this.dialog({
					id:"tipDialog",
					className:"ui-tipDialog",
					bgSwitch:false,
					tip:true,
					closeSwitch:false,
					bgFn:false,
					content:"<div class='ui-tipDialog-con'>"+msg+"</div>"
				},"msg");
				_msgDialog.open();
			},
			msgDialog:function(msg){
				var _msgDialog=this.dialog({
					id:"msgDialog",
					className:"ui-msgDialog",
					bgSwitch:false,
					closeSwitch:false,
					bgFn:false,
					content:"<div class='ui-msgDialog-con'>"+msg+"</div>",
					footer:[
						{name:"确定",className:"ui-msgDialogBtn"}
					],
					blindEvent:[
						{
							ele:".ui-msgDialogBtn",
			...
			simpleDialog:function(msg,sureFunction,cancleFunction){
				var _simpleDialog=this.dialog({
					id:"simpleDialog",
					className:"ui-simpleDialog",
					bgSwitch:true,
					closeSwitch:false,
					bgFn:false,
					content:"<div class='ui-simpleDialog-con'>"+msg+"</div>",
					footer:[
						{name:"确定",className:"ui-simpleDialogSureBtn"},
						{name:"取消",className:"ui-simpleDialogCancleBtn"}
					],
					blindEvent:[
						{
							ele:".ui-simpleDialogSureBtn",
							type:"click",
							fn:function(){
